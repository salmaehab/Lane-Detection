...
mode = sys.argv[4]
def process_video(video, input_path, output_path, mode):
        
        if mode == '0': # debugging mode
            if video == '1': 
            ...
            opencv_rgb_img = cv2.cvtColor(sora2, cv2.COLOR_GRAY2BGR)
            ...
            plt.figure(figsize = (20, 15))
            plt.imshow(out_img)
            plt.savefig('out_img2.jpg')
        if mode == '1': # running mode
            if video =='1':
                clip = VideoFileClip(input_path)
                out_clip = clip.fl_image(main_forward)
                out_clip.write_videofile(output_path, audio=False)
                ...
                sora = plt.imread(input_path)
                sora = cv2.warpPerspective(sora, M, (1280, 720), flags=cv2.INTER_LINEAR) # warp image 
                sora =  threshold_forward(sora) # image with lane lines detected
                sora = lane_line_forward(sora) # image with lanes detected
                sora = cv2.warpPerspective(sora, M_inv, (1280, 720), flags=cv2.INTER_LINEAR) # el image b3d mareg3et l2slaha
                out_img = np.copy(plt.imread(input_path))
                out_img = cv2.addWeighted(out_img, 1, sora, 0.6, 0)
                plt.figure(figsize = (20, 15))
                plt.imshow(out_img)
                plt.savefig(output_path)
            
process_video(video, input_path, output_path, mode)
